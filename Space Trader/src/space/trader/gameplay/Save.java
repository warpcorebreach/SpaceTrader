...
import space.trader.location.SystemStats;
...
public class Save {
     ...
     * Gets the current Player, SolarSystem, and Universe
     */
    public Save() {
    ...
    public void saveTextFile() {
        try {
            try (FileWriter writer = new FileWriter(new File("save.txt"))) {
                if (sys == null) {
                    String temp = "InSpace" + "," + "NONE" + "," + "NONE" + ","
                            + "25" + "," + "75" + "," + "0";
        ...
        try {
            try (FileWriter writer = new FileWriter(new File("universe.txt"))) {
    ...
    public void loadTextFile() {
        try {
            ...
            player = new Player(saveData[6], Integer.parseInt(saveData[7]),
                    Integer.parseInt(saveData[8]),
                    Integer.parseInt(saveData[9]),
                    Integer.parseInt(saveData[10]),
                    Integer.parseInt(saveData[11]));
            ...
            int i = 1;
            while (i < maxIndex) {
                s = new SolarSystem(uniData[i],
                        Integer.parseInt(uniData[i + 3]),
        ...
        try (ObjectInputStream in = new ObjectInputStream(
                new FileInputStream("ship.bin"))) {
            player.setShip((Ship) in.readObject());
        } catch (IOException e) {
            System.out.println("file not found");
        } catch (ClassNotFoundException ex) {
            System.out.println("class not found - if you are seeing this"
